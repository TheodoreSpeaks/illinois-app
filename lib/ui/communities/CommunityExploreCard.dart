...
class CommunityExploreCard extends StatefulWidget {
  ...
  @override
  _CommunityExploreCardState createState() => _CommunityExploreCardState();
}

class _CommunityExploreCardState extends State<CommunityExploreCard>
    implements NotificationsListener {
  static const EdgeInsets _detailPadding =
  ...
  String get semanticLabel {
    ...
    String locationText = ExploreHelper.getShortDisplayLocation(
            widget.explore, widget.locationData) ??
        "";
    String workTime =
        ((explore is Dining) ? explore.displayWorkTime : null) ?? "";
    int eventConvergeScore = (explore is Event) ? explore.convergeScore : null;
    String convergeScore = ((eventConvergeScore != null)
            ? (eventConvergeScore.toString() + '%')
            : null) ??
        "";
    String interests =
        ((explore is Event) ? _getInterestsLabelValue() : null) ?? "";
    interests = interests.isNotEmpty
        ? interests.replaceRange(
            0,
            0,
  ...
  Widget build(BuildContext context) {
    ...
    String imageUrl =
        AppString.getDefaultEmptyString(value: widget.explore.exploreImageURL);
...
//          excludeSemantics: true,
                    ...
                    child: Container(
                      ...
                      child: Column(
                        ...
                        children: <Widget>[
                          ...
                          Visibility(
                            visible: isCompositeEvent,
                            child: Container(
                              height: _EventSmallCard._cardHeight,
                            ),
                          )
                        ],
                      ),
  ...
  bool _showInterests() {
    String interestsLabelValue = _getInterestsLabelValue();
    return (widget.explore is Event) &&
        AppString.isStringNotEmpty(interestsLabelValue);
  }
  ...
  Widget _buildConvergeButton() {
    if (_showInterests() || !_hasConvergeContent()) return Container();
    ...
    return Container(
        ...
        child: Column(children: <Widget>[
          ...
          Padding(
              ...
              child: ExploreConvergeDetailButton(
                eventConvergeScore: _getConvergeScore(),
                eventConvergeUrl: _getConvergeUrl(),
              ))
        ]));
  }
  ...
  Widget _exploreTop() {
    ...
    if (event != null) {
      ...
      leftLabelStyle =
          TextStyle(fontSize: 18, color: Styles().colors.fillColorPrimary);
    }
    ...
    return Row(
      children: <Widget>[
        Expanded(
          child: Padding(
              padding:
                  EdgeInsets.only(left: 16, right: 16, top: 19, bottom: 12),
              child: Text(
                leftLabel,
  ...
  Widget _exploreName() {
    return Padding(
      ...
      child: Text(
        (widget.explore.exploreTitle != null)
  ...
  Widget _exploreWorkTimeDetail() {
    Dining dining =
        (widget.explore is Dining) ? (widget.explore as Dining) : null;
  ...
  Widget _explorePaymentTypes() {
    ...
    if ((paymentTypes != null) && (0 < paymentTypes.length)) {
      ...
      for (PaymentType paymentType in paymentTypes) {
        ...
        if (image != null) {
          details
              .add(Padding(padding: EdgeInsets.only(right: 6), child: image));
        }
      }
    }
  ...
  Widget _topBorder() {
    return widget.showTopBorder
        ? Container(height: 7, color: widget.explore.uiColor)
        : Container();
  }

  String _getInterestsLabelValue() {
    return (!widget.hideInterests && (widget.explore is Event))
        ? ((widget.explore as Event).displayInterests)
        : "";
  }
  ...
  Widget _buildCompositeEventsContent(bool isCompositeEvent) {
    ...
    List<Event> subEvents =
        parentEvent.recurringEvents ?? parentEvent.featuredEvents;
    ...
    _EventCardType type =
        parentEvent.isSuperEvent ? _EventCardType.sup : _EventCardType.rec;
            ...
            itemBuilder: (context, index) {
              if (index == 0 || index == itemsCount - 1) {
                ...
                return _EventSmallCard(
                  type: _EventCardType.more,
                  onTap: () => _onTapSmallExploreCard(
                      context: context,
                      parentEvent: parentEvent,
                      cardType: _EventCardType.more),
                );
              }
  ...
  void _onTapExploreCardStar() {
    Analytics.instance.logSelect(
        target:
            "ExploreCard mark favorite explore: ${widget.explore.exploreId}");
    Event event = (widget.explore is Event) ? (widget.explore as Event) : null;
    if (event?.isRecurring ?? false) {
      ...
      Favorite favorite =
          widget.explore is Favorite ? widget.explore as Favorite : null;
      User().switchFavorite(favorite);
    }
  }

  void _onTapSmallExploreCard(
      {BuildContext context,
      _EventCardType cardType,
      Event parentEvent,
      Event subEvent}) {
    if (cardType == _EventCardType.more) {
      if (parentEvent.isSuperEvent == true) {
        Navigator.push(
            ...
            CupertinoPageRoute(
                builder: (context) =>
                    EventsSchedulePanel(superEvent: parentEvent)));
      } else {
        Navigator.push(
            context,
            CupertinoPageRoute(
      ...
      Navigator.push(
          ...
          CupertinoPageRoute(
              builder: (context) => ExploreEventDetailPanel(
                  event: subEvent, superEventTitle: parentEvent.title)));
    }
  }
...
class _EventSmallCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    double internalHeight =
        _cardHeight - (borderWidth * 2 + internalPadding * 4 + topBorderHeight);
    return GestureDetector(
      onTap: onTap,
      child: Semantics(
                              ...
                              Visibility(
                                ...
                                child: GestureDetector(
                                    ...
                                    onTap: () {
                                      ...
                                      Analytics.instance.logSelect(
                                          target:
                                              "ExploreCard mark favorite explore: $favoriteId");
                                      User().switchFavorite(favorite);
                                    },
                                    child: Semantics(
                                        label: isFavorite
                                        ...
                                        child: Container(
                                            child: Padding(
                                                padding:
                                                    EdgeInsets.only(left: 24, bottom: 5),
                                                child: Image.asset(isFavorite ? 'images/icon-star-selected.png' : 'images/icon-star.png'))))),
                              ),
                              Visibility(
                                visible: isMoreCardType,
                                child: Padding(
    ...
    switch (type) {
      ...
      case _EventCardType.more:
        return Localization().getStringEx(
            'widget.explore_card.small.view_all.title', 'View all events');
      default:
        return '';
    }
  }
