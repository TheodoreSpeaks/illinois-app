...
class FirebaseMessaging with Service implements NotificationsListener {
  static const String notifyToken =
      "edu.illinois.rokwire.firebase.messaging.token";
  static const String notifyPopupMessage =
      "edu.illinois.rokwire.firebase.messaging.message.popup";
  static const String notifyScoreMessage =
      "edu.illinois.rokwire.firebase.messaging.message.score";
  static const String notifyConfigUpdate =
      "edu.illinois.rokwire.firebase.messaging.config.update";
  static const String notifyPollOpen =
      "edu.illinois.rokwire.firebase.messaging.poll.create";
  static const String notifyEventDetail =
      "edu.illinois.rokwire.firebase.messaging.event.detail";
  static const String notifyAthleticsGameStarted =
      "edu.illinois.rokwire.firebase.messaging.athletics_game.started";
  static const String notifySettingUpdated =
      "edu.illinois.rokwire.firebase.messaging.setting.updated";
  ...
  Future<bool> subscribeToTopic(String topic) async {
    ...
    try {
      String url = (Config().sportsServiceUrl != null)
          ? "${Config().sportsServiceUrl}/api/subscribe"
          : null;
      String body = json.encode({'token': _token, 'topic': topic});
      Response response =
          await Network().post(url, body: body, auth: NetworkAuth.App);
  ...
  Future<bool> unsubscribeFromTopic(String topic) async {
    ...
    try {
      String url = (Config().sportsServiceUrl != null)
          ? "${Config().sportsServiceUrl}/api/unsubscribe"
          : null;
      String body = json.encode({'token': _token, 'topic': topic});
      Response response =
          await Network().post(url, body: body, auth: NetworkAuth.App);
  ...
  Future<bool> send({String topic, dynamic message}) async {
    try {
      String url = (Config().sportsServiceUrl != null)
          ? "${Config().sportsServiceUrl}/api/message"
          : null;
      ...
      try {
        if (Platform.isIOS) {
          ...
          if (AppString.isStringNotEmpty(title) ||
              AppString.isStringNotEmpty(body)) {
            ...
            LocalNotifications().showNotification(
                title: title, message: body, payload: notificationPayload);
          } else if (data != null) {
  ...
  bool get notifyEventReminders {
    return _getNotifySetting('event_reminders');
  }

  set notifyEventReminders(bool value) {
    _setNotifySetting('event_reminders', value);
  }

  bool get notifyAthleticsUpdates {
    return _getNotifySetting('athletic_updates');
  }

  set notifyAthleticsUpdates(bool value) {
    _setNotifySetting('athletic_updates', value);
  }

  bool get notifyDiningSpecials {
    return _getNotifySetting('dining_specials');
  }

  set notifyDiningSpecials(bool value) {
    _setNotifySetting('dining_specials', value);
  }
  ...
  void _setNotifySetting(String name, bool value) {
    if (_notifySettingsAvailable && (_getNotifySetting(name) != value)) {
      ...
      // _processNotifySettingSubscription(
      //     topic: _notifySettingTopics[name],
      //     value: value,
      //     subscribedTopis: subscribedTopis);
  ...
  void _updateRolesSubscriptions() {
    if (hasToken) {
      // _processRolesSubscriptions(
      //     subscribedTopis: Storage().firebaseSubscriptionTopis);
    }
  }

  void _updateNotifySettingsSubscriptions() {
    if (hasToken) {
      // _processNotifySettingsSubscriptions(
      //     subscribedTopis: Storage().firebaseSubscriptionTopis);
    }
  }
  ...
  void _updateAthleticsSubscriptions() {
    if (hasToken) {
      // _processAthleticsSubscriptions(
      //     subscribedTopis: Storage().firebaseSubscriptionTopis);
    }
  }
  ...
  void _processPermanentSubscriptions({Set<String> subscribedTopis}) {
    for (String permanentTopic in _permanentTopis) {
      if ((subscribedTopis == null) ||
          !subscribedTopis.contains(permanentTopic)) {
  ...
  void _processRolesSubscriptions({Set<String> subscribedTopis}) {
    ...
    for (UserRole role in UserRole.values) {
      ...
      bool roleSubscribed =
          (subscribedTopis != null) && subscribedTopis.contains(roleTopic);
  ...
  void _processNotifySettingsSubscriptions({Set<String> subscribedTopis}) {
    _notifySettingTopics.forEach((String setting, String topic) {
      ...
      _processNotifySettingSubscription(
          topic: topic, value: value, subscribedTopis: subscribedTopis);
    });
  }

  void _processNotifySettingSubscription(
      {String topic, bool value, Set<String> subscribedTopis}) {
    if (topic != null) {
      bool itemSubscribed =
          (subscribedTopis != null) && subscribedTopis.contains(topic);
