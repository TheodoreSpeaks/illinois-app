...
class GeoFence with Service implements NotificationsListener {
  static const String notifyCurrentRegionsUpdated =
      "edu.illinois.rokwire.geofence.regions.current.updated";
  static const String notifyCurrentBeaconsUpdated =
      "edu.illinois.rokwire.geofence.beacons.current.updated";
  ...
  Future<void> initService() async {
    ...
    _regions =
        _useAssets ? _loadRegionsFromAssets() : await _loadRegionsFromCache();
    if (_regions != null) {
      _updateRegions();
    } else {
  ...
  List<GeoFenceRegion> regionsList(
      {String type, bool enabled, GeoFenceRegionType regionType, bool inside}) {
            ...
            ((inside == null) ||
                (inside ==
                    ((_currentRegions != null) &&
                        _currentRegions.contains(regionId))))) {
          regions.add(region);
        }
      });
    }
  ...
  Future<void> _updateRegions() async {
    ...
    LinkedHashMap<String, GeoFenceRegion> regions =
        _regionsFromJsonString(jsonString);
    if ((regions != null) && !_areRegionsEqual(_regions, regions)) {
      // DeepCollectionEquality().equals(_regions, regions)
      _regions = regions;
      _monitorRegions();
      _saveRegionsStringToCache(jsonString);
    }
  ...
  static LinkedHashMap<String, GeoFenceRegion> _regionsFromJsonString(
      String jsonString) {
    List<dynamic> jsonList = AppJson.decode(jsonString);
    return (jsonList != null) ? GeoFenceRegion.mapFromJsonList(jsonList) : null;
  }

  static bool _areRegionsEqual(LinkedHashMap<String, GeoFenceRegion> regions1,
      LinkedHashMap<String, GeoFenceRegion> regions2) {
    ...
    if (_regions != null) {
      _regions.forEach((String regionId, GeoFenceRegion region) {
        if (region?.enabled ?? false) {
          regionsForMonitor
              .add(region.toJson(locationRadius: debugRadius?.toDouble()));
        }
      });
    }
  ...
  void _updateCurrentRegions(dynamic param) {
    Set<String> currentRegions =
        (param != null) ? Set.from(param).cast<String>() : Set();
  ...
  void _updateCurrentBeacons(dynamic param) {
    try {
      Map<String, dynamic> data =
          (param is Map) ? param.cast<String, dynamic>() : null;
      String regionId = (data != null) ? data['regionId'] : null;
      if (regionId != null) {
        List<GeoFenceBeacon> beacons =
            GeoFenceBeacon.listFromJsonList(data['beacons']);
        if (beacons != null) {
